...
contains
  ...
  subroutine win__scan_buf(wh, buf)
    ...
    ichk = int( transfer(buf(1:1), idb) )  !2021-07-12 N. Uchida check first
    ichk2 = int( transfer(buf(2:2), idb) ) ! 4 byte because hear WIN1 is channel
    ichk3 = int( transfer(buf(3:4), idb) ) ! header but WIN32 '0000'
!c    print *,"ichk= ",ichk,ichk2,ichk3
    ...
    if( YR_MIN <= yr_32 .and. yr_32 <= YR_MAX .and. &
             ...
             0 <= mi_32 .and. mi_32 <= 59     .and. &
                ichk == 0 .and. ichk2 == 0 .and. &
                ichk3 == 0 ) then
      wh%fmt = WIN32
      pb(1) = 5
